...
[[nodiscard]] std::size_t count_utf8_codepoints(std::string_view text) noexcept
{
    std::size_t count = 0U;
...
[[nodiscard]] guinevere::ui::TextEditEchoMode resolve_echo_mode(
    int mode_index,
    std::size_t show_in_duration_ms
) noexcept
{
    switch(mode_index) {
    case 0:
        return guinevere::ui::TextEditEchoMode::normal();
    case 2:
        return guinevere::ui::TextEditEchoMode::no_echo();
    case 3:
        return guinevere::ui::TextEditEchoMode::show_in(
            static_cast<std::uint32_t>(show_in_duration_ms)
        );
    case 1:
    default:
        return guinevere::ui::TextEditEchoMode::password();
    }
}

[[nodiscard]] std::string echo_mode_label(int mode_index)
{
    switch(mode_index) {
    case 0:
        return "Normal";
...
class PasswordEditorPanel {
public:
    void render(guinevere::ui::ComponentScope& component) const
    {
        const std::string& password_utf8 = component.state().use<std::string>(
            "password_utf8",
            std::string("hunter2")
        );
        const std::string& submitted_password_utf8 =
            component.state().use<std::string>("submitted_password_utf8", std::string{});
        const int echo_mode_index = component.state().use<int>("echo_mode_index", 1);
        const std::size_t show_in_duration_ms =
            component.state().use<std::size_t>("show_in_duration_ms", 900U);
        ...
        const std::size_t current_length = count_utf8_codepoints(password_utf8);
        const std::size_t submitted_length = count_utf8_codepoints(submitted_password_utf8);
        const std::string submitted_label = submitted_password_utf8.empty()
            ? std::string("Press Enter or click Submit password to snapshot its current length")
            : std::string("Last submitted length: ") + std::to_string(submitted_length) + " chars";
        ...
        panel_entry.min_height(280.0f);

        component.label(panel_key, "PasswordLine Text Input");

        auto text_edit_entry = component.text_edit(panel_key, password_utf8);
        text_edit_entry.input_type(guinevere::ui::TextEditInputType::PasswordLine);
        text_edit_entry.echo_mode(resolve_echo_mode(echo_mode_index, show_in_duration_ms));
        ...
        if(custom_selection_background) {
            text_edit_entry.selection_background_color(
                guinevere::gfx::Color{0.42f, 0.20f, 0.07f, 0.92f}
            );
        }
        if(custom_selection_text) {
            text_edit_entry.selection_text_color(
                guinevere::gfx::Color{1.0f, 0.96f, 0.88f, 1.0f}
            );
        }
        text_edit_entry.on_text_change([component](const std::string& changed_password_utf8) mutable {
            component.state().set<std::string>("password_utf8", changed_password_utf8);
        });
        text_edit_entry.on_text_submit([component](const std::string& submitted_value_utf8) mutable {
            component.state().set<std::string>("submitted_password_utf8", submitted_value_utf8);
        });

        auto cycle_echo_mode_button = component.button(
            panel_key,
            std::string("Echo mode: ") + echo_mode_label(echo_mode_index)
        );
        cycle_echo_mode_button.on_click([component]() mutable {
            component.state().update<int>("echo_mode_index", [](int& value) {
                value = (value + 1) % 4;
            });
        });

        auto cycle_show_in_duration_button = component.button(
            panel_key,
            std::string("ShowIn duration: ") + std::to_string(show_in_duration_ms) + " ms"
        );
        cycle_show_in_duration_button.on_click([component]() mutable {
            component.state().update<std::size_t>("show_in_duration_ms", [](std::size_t& value) {
                if(value == 350U) {
                    value = 900U;
                    return;
                }
        ...
        auto submit_button = component.button(panel_key, "Submit password");
        submit_button.on_click([component, password_utf8]() mutable {
            component.state().set<std::string>("submitted_password_utf8", password_utf8);
        });

        auto clear_button = component.button(panel_key, "Clear password");
        clear_button.on_click([component]() mutable {
            component.state().set<std::string>("password_utf8", std::string{});
        });
        ...
        auto toggle_ctrl_c_button = component.button(
            panel_key,
            std::string("Ctrl+C request: ") + (allow_ctrl_c ? "ON" : "OFF") + " (ignored)"
        );
        ...
        auto toggle_ctrl_x_button = component.button(
            panel_key,
            std::string("Ctrl+X (Delete selection): ") + (allow_ctrl_x ? "ON" : "OFF")
        );
        ...
        component.label(
            panel_key,
            std::string("Current password length: ") + std::to_string(current_length) + " chars"
        );
        component.label(panel_key, submitted_label);
        component.label(
            panel_key,
            "PasswordLine: Ctrl+C never copies | Ctrl+X deletes the selection without filling clipboard"
        );
        component.label(
            panel_key,
            "Echo modes: Normal | Password | NoEcho | ShowIn(N ms). Default mode is Password."
        );
        component.label(
            panel_key,
            "Left/Right move | Ctrl+A/V/X | Mouse drag select | Insert toggles caret | Enter submits"
        );
    }
};
...
int main()
{
    ...
    PasswordEditorPanel password_editor;
    ui_runtime.reserve(18U);

    guinevere::app::RunConfig config;
    config.width = 920;
    config.height = 760;
    config.title = "Guinevere TextEdit Password Demo";
    ...
    const auto render_frame = [&](
            ...
            const std::string footer_text =
                "PasswordLine demo with echo_mode, no-copy behavior, and configurable input handling. Breakpoint: "
            ...
            (void)app_component.label(header_key, "Guinevere TextEdit Password Demo");
            ...
            layout_root_entry.main_axis_tracks({
                guinevere::ui::Flex(1.0f).min(260.0f).pref(260.0f).priority(1),
            ...
            app_component.mount_component(
                app_component.auto_local_key("password_editor"),
                layout_root_key,
                password_editor
            );
    ...
    callbacks.on_frame = [&](guinevere::app::Context& context) {
        ...
        return ui_runtime.component_frame(
            ...
            "demo_textedit_password",
            render_frame,
            guinevere::gfx::Color{0.09f, 0.08f, 0.11f, 1.0f}
        );
    };
